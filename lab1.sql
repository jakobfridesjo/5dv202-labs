...
ON products.supplier_id=suppliers.supplier_id AND suppliers.company_name='Tokyo Traders';
...
*/
SELECT product_name FROM products WHERE product_id IN (SELECT product_id FROM order_details WHERE order_id IN (SELECT order_id from order_details WHERE product_id IN (SELECT product_id FROM products WHERE product_name='Ipoh Coffee')));
...
/* Make string of manager name */
(SELECT CONCAT(first_name, ' ', last_name) FROM employees WHERE title LIKE '%Manager%');

/* Get employees of manager */
(SELECT * FROM employees WHERE reports_to=(SELECT employee_id FROM employees WHERE title LIKE '%Manager%'));

/* Get orders made from employees */
SELECT * FROM orders WHERE orders.employee_id IN (SELECT employee_id FROM employees WHERE reports_to=(SELECT employee_id FROM employees WHERE title LIKE '%Manager%'));

/* Get order details for order of a employee */
SELECT unit_price,quantity,discount FROM order_details WHERE order_id IN (SELECT order_id FROM orders WHERE orders.employee_id IN (SELECT employee_id FROM employees WHERE reports_to=(SELECT employee_id FROM employees WHERE title LIKE '%Manager%')));

/* Get revenue */
SELECT SUM(unit_price*quantity*(1-discount)) FROM order_details WHERE order_id IN (SELECT order_id FROM orders WHERE orders.employee_id IN (SELECT employee_id FROM employees WHERE reports_to=(SELECT employee_id FROM employees WHERE title LIKE '%Manager%')));

/* Get employee with top revenue */
